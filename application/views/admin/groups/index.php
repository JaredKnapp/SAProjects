    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body form">
                ...
                <?php echo form_close(); ?>
                <div>
                    <table id="membertable" class="table table-hover table-striped table-bordered" cellspacing="0">
                        <thead>
                            <tr>
                                <th>id</th>
                                <th>User</th>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        membertable = $('#membertable').DataTable({
            ...
            "searching": false,
            "ajax": {
                url: "<?php echo site_url('admin/groupmembers/ajax_index')?>",
                type: "POST"
            },
            "columnDefs": [
                { "name": "id", "targets": 0, "visible": false },
                { "name": "user", "targets": 1, "width": "50%" },
                {
                    "name": "actions", "targets": 3, "orderable": false, "width": "25%",
                    "render": function (data, type, row) {
                        editButton = '<a class="btn btn-sm btn-success" href="javascript:void(0)" title="Edit" onclick="edit_member(\'' + row[0] + '\')"><i class="fa fa-pencil fa-fw"></i></a>';
                        deleteButton = '<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Delete" onclick="delete_member(\'' + row[0] + '\')"><i class="fa fa-trash-o fa-fw"></i></a>';
                        return editButton + '&nbsp;' + deleteButton;
                    }
                }
            ]
    ...
    function add_member() {
        member_save_method = 'add';
        $('#member-form')[0].reset();
        $('.member-form-group').removeClass('has-error');
        $('.member-help-block').empty();

        $('.member-modal-title').text('New Group');
        $('#member_modal_form').modal('show');
    }

    function edit_member(id, newStatus) {
        member_save_method = 'update';
        $('#member-form')[0].reset();
        $('.member-form-group').removeClass('has-error');
        $('.member-help-block').empty();

        $.ajax({
            url: "<?php echo site_url('admin/groupmembers/ajax_edit/')?>/" + id,
            cache: false,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
    ...
    function save_member() {
        ...
        if (save_method == 'add') {
            url = "<?php echo site_url('admin/groupmembers/ajax_add')?>";
        } else {
            url = "<?php echo site_url('admin/groupmembers/ajax_update')?>";
        }
    ...
    function delete_member(id) {
        if (confirm('Are you sure you wish to delete this Group?')) {
            $.ajax({
                url: "<?php echo site_url('admin/groupmembers/ajax_delete')?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data) {
                    //if success reload ajax table
